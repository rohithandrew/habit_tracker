...
export function TimerView({ userId, readOnly = false }: { userId: string; readOnly?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [active, history] = await Promise.all([
        ...
        fetchSessionHistory(userId, since),
      ]);
  ...
  async function startWithTask(taskDescription: string) {
    if (readOnly || !taskDescription.trim()) return;
    setStarting(true);
    try {
      const created = await startSession(userId, taskDescription.trim());
  ...
  return (
    <ScrollView style={styles.flexOne} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      ...
      {activeSession ? (
        ...
        <>
          {todaysTasks.length > 0 ? (
            <Card style={styles.card}>
              <ThemedText type="sectionTitle">Continue today&apos;s work</ThemedText>
          ...
          <Card style={styles.card}>
            <View style={styles.cardTitleRow}>
              ...
              <ThemedText type="sectionTitle">Start something new</ThemedText>
            </View>
            <TextField
      ...
      <View style={styles.sectionHeader}>
        <ThemedText type="sectionTitle">Recent sessions</ThemedText>
        {!readOnly ? (
      ...
      <Card style={styles.recentCard}>
        {loading ? (
          ...
          <ThemedText themeColor="textSecondary">
            {readOnly ? 'No sessions yet.' : 'No sessions yet — start your first one above.'}
          </ThemedText>
        ) : (
